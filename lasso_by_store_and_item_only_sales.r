units_sold = read.csv('WMT_units_sold.csv')[,4:44]
...
for (str_nbr in seq(1:45)) {
  store_subset = subset(units_sold, units_sold$store_nbr == str_nbr)
...
write.csv(store_coef_table, file = 'store_coef_table_units_sold.csv', row.names = FALSE)


## Lasso - Units Sold by Item_Nbr
item_coef_table = NULL
for (product_number in seq(1:111)) {
  store_subset = subset(units_sold, units_sold$item_nbr == product_number)
  x = model.matrix(units~.-store_nbr-item_nbr, store_subset)
  y = store_subset$units
...
write.csv(item_coef_table, file = 'item_coef_table_units_sold.csv', row.names = FALSE)
